...
public class FileUtils {
	...
	public static boolean copy(FileSystem srcFS, Path src, 
			Path dst, String queryStr,boolean deleteSource, boolean overwrite,
		...
		try{
		result= copy(srcFS, fileStatus,  dst,queryStr, deleteSource, overwrite,
				conf,ftpClient);
		}catch(Exception e){
			e.printStackTrace();
		} finally {
	...
	public static boolean copy(FileSystem srcFS, FileStatus srcStatus,
			 Path dst, String queryStr,boolean deleteSource,
			boolean overwrite, Configuration conf,FTPClient ftpClient) throws IOException, ParseException {
		...
		if (srcStatus.isDirectory()) {//若原路径是一个目录
			...
			if(queryStr!=null){
				...
				Log.info("filter time:"+(System.currentTimeMillis()-begin)+" ms");
			}
			Log.info("file total count:"+contents.length);
			for (int i = 0; i < contents.length; i++) {
				copy(srcFS, contents[i],  new Path(dst, contents[i]
						.getPath().getName()),queryStr, deleteSource, overwrite, conf,ftpClient);
				if(i%100==0||i==contents.length-1){
					Log.info("have transfered "+(i+1)+" files");
				}
			}
		} else {//若是文件，直接上传
			InputStream in = null;
			try {
				...
				Log.debug("transfered filename :"+dstPath);
			} catch (IOException e) {
